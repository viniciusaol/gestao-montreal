                 ...
                 SELECT b.booking_id,
                    ...
                    COALESCE(bmpd.min_payment_date, fv.pay_date) AS pay_date,
                        CASE
                            ...
                            WHEN (fi.description ~~* '%Leandro Bonete%'::text OR fi.description ~~* '%Leandro B.%'::text OR fi.description ~~* '%Leandro%'::text OR fi.description ~~* '%17/09/2026%21:00%'::text OR fi.description ~~* '%AULA DE REPOSIÇÃO%'::text) THEN 'Leandro Bonete'::text
                 ...
                 SELECT p.customer_code,
                    ...
                    b.start_time,
                        CASE
                            ...
                            WHEN (b.booking_id = 11830 OR b.description ~~* '%Leandro Bonete%'::text OR b.description ~~* '%Leandro B.%'::text OR b.description ~~* '%Leandro%'::text) THEN 'Leandro Bonete'::text
...
END, 'Sem professor'::text) AS professor
                ...
                UNION ALL
                 ...
                 SELECT c.booking_id,
                    ...
                    c.customer_code
                   FROM all_combined c
                  WHERE ((EXTRACT(isodow FROM c.booking_date) = (6)::numeric) AND (c.start_time IS NOT NULL AND c.start_time = '10:00:00'::time without time zone) AND (c.professor = 'Leandro Bonete'::text))
                UNION ALL
                 SELECT c.booking_id,
                    c.booking_date,
                    c.is_socio_benefit,
                 ...
                 SELECT all_combined.booking_id,
                   ...
                   FROM all_combined
                  WHERE (NOT ((EXTRACT(isodow FROM all_combined.booking_date) = (6)::numeric) AND (all_combined.start_time IS NOT NULL AND all_combined.start_time = '10:00:00'::time without time zone) AND (all_combined.professor = 'Leandro Bonete'::text)))
                UNION ALL
