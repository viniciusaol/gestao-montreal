...
sql = """CREATE OR REPLACE VIEW public.vw_mt_comissoes_detalhadas AS
WITH view_base AS (
                ...
                UNION ALL
                 ...
                 SELECT rf.item_key,
                  ...
                  WHERE ((rf.paid = true) AND (NOT (EXISTS ( SELECT 1
                          ...
                          WHERE (lcm.item_key = rf.item_key)))) AND (rf.item_canceled = false) AND (rf.sale_canceled = false) AND (COALESCE(rf.sale_type, ''::text) <> 'refund'::text) AND (rf.valor_faturamento > (0)::numeric) AND ((rf.categoria = 'Aulas'::text) OR ((rf.categoria = 'Outros'::text) AND ((rf.description ~~* '%TÊNIS%'::text) OR (rf.description ~~* '%AULA%'::text) OR (rf.description ~~* '%GRUPO FIXO%'::text)) AND ((rf.description ~~* '%ADULTO%'::text) OR (rf.description ~~* '%KIDS%'::text) OR (rf.description ~~* '%AVULSA%'::text) OR (rf.description ~~* '%GRUPO FIXO%'::text)))))
                ), all_combined AS (
...
with open(r"c:\Users\vinic\.antigravity-ide\Gestão Montreal\scratch\perfect_view.sql", "w", encoding="utf-8") as f:
    f.write(sql)

print("Saved clean perfect_view.sql with exact matching logic!")
