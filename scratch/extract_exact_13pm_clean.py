...
def_str = parsed[0]['view_definition'].strip().rstrip(';')

full_sql = f"CREATE OR REPLACE VIEW public.vw_mt_comissoes_detalhadas AS\n{def_str};"

with open('scratch/exact_13pm_clean.sql', 'w', encoding='utf-8') as f_out:
    f_out.write(full_sql)

print("Saved scratch/exact_13pm_clean.sql. Length:", len(full_sql))
