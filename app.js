...
if (isCurrentRealMonth) {
  ...
  for (let d = startDay; d <= daysInMonth; d++) {
    ...
    if (importedForMonth.length > 0) {
      // Block 1 (Exact imported D30 for this day) + Block 2 (D0 remaining with Pace Ratio)
      const importedForDay = importedForMonth.filter(r => r.data_liberacao === dayStr).reduce((sum, r) => sum + (parseFloat(r.valor) || 0.0), 0.0);
      const dayD0Pace = includeInflows ? round2((remainingD0ToReceive * paceRatio) * (dayWeight / remainingDaysWeight)) : 0.0;
      totalInflowDay = round2(importedForDay + dayD0Pace);
    } else {
      // Fallback: distribute D30 and D0 proportionally over remaining days with Pace Ratio
