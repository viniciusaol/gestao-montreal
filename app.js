...
const projCommissionRate = 0.47; // 47% Real Average Teacher Commission Rate
...
if (isCurrentRealMonth) {
  ...
  const expectedMonthCommission = round2(107165.87 * projCommissionRate);
  totalProjCommission = Math.max(actualPaidCommission, expectedMonthCommission);
} else {
  totalProjCommission = round2(107165.87 * projCommissionRate);
}
  ...
  for (let d = startDay; d <= daysInMonth; d++) {
    ...
    // Block 1: Recebimentos D30 das Vendas do Mês Anterior (Julho - Agenda Importada ou Estimativa)
    const importedForMonth = (allImportedReceivablesData || []).filter(r => r.data_liberacao && r.data_liberacao.startsWith(mKey));
    let block1_D30 = 0.0;
    if (importedForMonth.length > 0) {
    ...
    // Block 2: Projeção de Vendas D0 do Próprio Mês Atual (Agosto - ~30%) com Pace de Desempenho
...
function renderGoalsDashboard(itemsData, courtData, totalHoursOcupadas, year, month, totalActiveStudents) {
  ...
  if (studentCardFooter) {
    ...
    studentCardFooter.innerHTML += ` <span style="display:block;font-size:0.72rem;color:var(--text-muted);margin-top:2px;">(${uniqueCount} alunos únicos)</span>`;
  }
